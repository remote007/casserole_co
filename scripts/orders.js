...
import createHourglassTimer from './hourglass.js'; // Import the hourglass script

const ordersPage = (() => {
    const deliveryDuration = 30 * 60 * 1000; // 30 minutes in milliseconds
    ...
    const displayOrders = (userOrders) => {
        ...
        const createOrderCard = (order, isMain = false) => {
            ...
            orderDiv.style.margin = isMain ? '0 auto 20px auto' : '0';
            orderDiv.style.padding = '20px';
            orderDiv.style.background = 'rgba(255, 255, 255, 0.5)';
            orderDiv.style.backdropFilter = 'blur(10px)';
            orderDiv.style.boxShadow = '0 8px 16px rgba(188, 166, 2, 0.93)';
            orderDiv.style.fontFamily = 'Arial, sans-serif';
            ...
            const remainingTime = calculateRemainingTime(order.time);
            const timeText = remainingTime > 0
                ? `Time Remaining: ${Math.ceil(remainingTime / 60000)} seconds`
                : 'Delivery Completed';

            const timeRemaining = document.createElement('p');
            timeRemaining.textContent = timeText;
            timeRemaining.style.color = remainingTime > 0 ? 'darkbrown' : 'green';
            timeRemaining.style.marginBottom = '12px';
            timeRemaining.style.display = 'none'; 

            const hourglassContainer = document.createElement('div'); // New hourglass container
            hourglassContainer.id = `hourglass-${order.id}`;
            hourglassContainer.style.margin = '20px auto';


            // if (order.delivered) {
            //     timeRemaining.style.display = 'none'; // Hide the timer for delivered orders
            // }

            const orderStatus = document.createElement('p');
            orderStatus.textContent = `Status: ${order.delivered ? 'Delivered' : "On it's way"}`;
            orderStatus.style.color = order.delivered ? 'green' : 'red';
            orderStatus.style.marginBottom = '12px';
            ...
            if (isMain) {
                ...
                if (!order.delivered && remainingTime > 0) 
                { 
                    orderDiv.appendChild(timeRemaining);
                    const hourglass = createHourglassTimer(remainingTime); // Pass remainingTime to hourglass
                    hourglassContainer.appendChild(hourglass);
                    orderDiv.appendChild(hourglassContainer);
                    
                } // Add hourglass to main card
                if (order.delivered && remainingTime <= 0) {
                    orderStatus.textContent = 'Status: Delivered';
